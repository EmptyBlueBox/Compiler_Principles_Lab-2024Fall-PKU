 ...
 * @note 寄存器分配原则: 每一个结果只被使用一次, 详细证明和说明如下
 * @note 因为每一行 koopa 代码只会使用 `@x`, `%1`, `1` 这样的值, 而这些值在 RISC-V 中要么在内存中, 要么就是立即数, 所以任意两个 koopa 指令之间是不会产生寄存器复用的
 * @note 而在同一行 koopa 代码中, 不可能展开为 `add a0, a0, a1;  add a2, a0, a3` (其中第二个指令的 a0 是第一个指令中第二个 a0 的值) 这样的指令, 所以寄存器复用也不会发生, 因此可以证明如下的寄存器分配策略是正确的
 * @note 寄存器分配策略:
 * @note 比如将立即数转移给 a0 寄存器, 我们现在就认为 a0 寄存器被占用了, 如果 a0 寄存器之后被调用了, 比如 `add a1, a0, a2` 或者 `sw a0, 0(sp)` , 这个 a0 被使用过了, 那么 a0 寄存器就会被标记为不被占用, 可以被另一个 RISC-V 指令作为存储数据的寄存器
 * @author Yutong Liang
 * @date 2024-11-28
 */
